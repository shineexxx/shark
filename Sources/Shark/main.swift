...
if let index = CommandLine.arguments.firstIndex(of: "--make-social-card") {
    let output = CommandLine.arguments.count > index + 1
        ? URL(fileURLWithPath: CommandLine.arguments[index + 1])
        : URL(fileURLWithPath: "social-card.png")
    do {
        try SocialCard.write(to: output)
        print("Карточка собрана: \(output.path)")
        exit(0)
    } catch {
